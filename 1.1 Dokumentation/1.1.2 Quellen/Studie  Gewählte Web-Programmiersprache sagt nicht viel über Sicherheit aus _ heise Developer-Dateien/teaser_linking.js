$(document).ready( function() {
  ...
  if (collection && collection.size() > 0) {
    collection.each( function() {
      // Diesen nicht nehmen, wenn es ein verschachtelter Teaser ist
      if ($(this).hasClass('teaser_artikel') && $(this).find('div.anriss_mit_bild_links')) {
        return 1;
      }

      // die explizit ausgezeichnete (class="the_content_url") URL suchen und verwenden
      var ahref_obj = $(this).find('a.the_content_url');
      var url_html, target, rel;

      // Fallback-Verhalten - die erste URL im Teaser nehmen
      var url = ahref_obj.attr('href');
      if (!url) { 
         ahref_obj = $(this).find('a');
         url       = ahref_obj.attr('href');
      }

      if (url) {
          target   = ahref_obj.attr('target');
          rel      = ahref_obj.attr('rel');
          url_html = '<a href="'+url+'"'+ (target ? ' target="'+ target +'"' : '') + (rel ? ' rel="'+ rel +'"' : '' ) + ' />';
      }
      ...
      $(this).find('img,p,span,dd').each ( function() {
        ...
        } else if ( $(this).is('img') && ( $(this).parent().is('a') || $(this).parent().is('p') ) ) {
                // IMGs sollten nicht verlinkt werden, wenn sie in einem P stehen
        } else {
          // DEFAULT: Treffen keine der o.g. Ausnahmen zu, dann wird alles mit 
          // dem href des Beitrags gewrapped.
          if ($(this).is('p') || $(this).is('dd')) {
              $(this).wrapInner(url_html);
          } else {
              $(this).wrap(url_html);
          }
        }   
