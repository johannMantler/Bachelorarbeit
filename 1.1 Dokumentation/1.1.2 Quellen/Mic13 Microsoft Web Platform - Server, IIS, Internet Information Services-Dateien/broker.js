...
if(SR_url.search(/www\.microsoft\.com\/windows\/pc-selector/i) !== -1 || SR_url.search(/(www|js|i3)\.microsoft\.com\/library\/svy\/(int_cle|int_cle_honestly)\.htm/i) !== -1) {	
		_sr_config = "broker-config_cle.js";
}else if(/[\w\.]+\/windowsphone\/en-us/i.test(SR_url)) {
	_sr_config = "broker-config_wp.js";	
}else if(/[\w\.]+\/learning\/en-/i.test(SR_url)) {
...
function _set_SessionCookie(_name, _val){
	if (_name == "captEmbed") {
             if(readCookie("captEmbed") != false) {
                   _val = readCookie("captEmbed") + "_" + _val;
             }
        }
